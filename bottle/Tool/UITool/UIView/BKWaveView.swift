...
class BKWaveViewConfigure: NSObject {
    /// 波浪颜色
    ...
    /// 默认配置
    fileprivate class func defaultConfigure() -> BKWaveViewConfigure {
        let configure = BKWaveViewConfigure()
        configure.y = configure.amplitude
        return configure
    }
}

class BKWaveView: UIView {
    
    private let configure = BKWaveViewConfigure.defaultConfigure()
    ...
    func updateWithConfigure(_ configureBlock: ((BKWaveViewConfigure) -> Void)?) {
